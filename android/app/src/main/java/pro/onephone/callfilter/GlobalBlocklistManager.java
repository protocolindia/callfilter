...
public class GlobalBlocklistManager {
    ...
    /** Enable or disable every reason in one shot (master toggle). */
    public synchronized void setAllReasonsEnabled(java.util.Collection<String> reasons, boolean enabled) {
        if (enabled) {
            for (String r : reasons) if (r != null && !r.isEmpty()) enabledReasons.add(r);
        } else {
            enabledReasons.clear();
        }
        saveEnabledReasons();
    ...
    /** True only the very first time the reasons screen is opened. */
    public synchronized boolean hasInitializedReasons() {
        return prefs.getBoolean("reasons_initialized", false);
    }

    public synchronized void markReasonsInitialized() {
        prefs.edit().putBoolean("reasons_initialized", true).commit();
    }
