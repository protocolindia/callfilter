...
/**
 * Lets the user pick contacts (with their phone numbers) for a schedule's
 * allowlist. The "allowed" list is returned to EditScheduleActivity via
...
public class ContactPickerActivity extends AppCompatActivity {
    ...
    /** All contacts loaded from device, may include multiple numbers per contact. */
    private final List<ContactEntry> contacts = new ArrayList<>();
    /** Filtered view of contacts (after search). */
    private final List<ContactEntry> visible = new ArrayList<>();
    /** Currently selected numbers and matching display names. */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (initNums != null) {
            for (int i = 0; i < initNums.size(); i++) {
                ...
                selected.put(normalize(n), name + "\u0000" + n);  // store name + raw number
            }
        }
