...
/**
 * Persists the user's schedules locally and syncs them to/from the backend.
 *
 * Overlap policy (per the user's spec): if multiple schedules are active for
 * the same moment, "most-recently-toggled wins." getActiveSchedule() returns
 * the active schedule with the latest lastToggledMs.
 */
public class ScheduleManager {
    ...
    public synchronized void save(Schedule s) {
        ...
        // Replace if exists by clientId, otherwise append
    ...
    public synchronized void delete(String clientId) {
        for (int i = cache.size() - 1; i >= 0; i--) {
            if (cache.get(i).clientId.equals(clientId)) {
                cache.remove(i);
            }
        }
        persist();
    ...
    /** Activate this schedule NOW for `minutes` minutes, regardless of its time window. */
    ...
    /**
     * Returns the active schedule at the given moment, or null if none.
     * If multiple are active, returns the one with the latest lastToggledMs
     * (overlap policy: most-recently-toggled wins).
     */
    ...
    /** Sync all schedules to backend in the background. */
                    ...
                    BackendClient.post(AuthManager.BACKEND_URL + "/api/schedules/sync", body,
                        new BackendClient.Callback() {
                            public void onResult(boolean ok, JSONObject resp, String err) {
                                Log.d(TAG, "Schedules sync ok=" + ok);
                            }
                        });
    ...
    /** Pull schedules from cloud — only if local list is empty (re-install scenario). */
    public void pullFromCloudIfEmpty() {
        ...
        BackendClient.get(url, new BackendClient.Callback() {
            public void onResult(boolean ok, JSONObject resp, String err) {
                ...
                Log.d(TAG, "Pulled " + arr.length() + " schedules from cloud");
            }
        });
    }
}
